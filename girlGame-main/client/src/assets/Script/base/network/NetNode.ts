...
import { EventManagerCallFunc } from "../common/EventManager";
...
interface CallBackTargetHttp {
    mod: string,
    type: string,
    handler: EventManagerCallFunc,
    target: any
}
...
export class NetNode {
    ...
    private static _eventListeners: { [key: string]: CallBackTargetHttp } = {};
    ...
    public connect(options: NetConnectOptions, connectedCallBack = null): boolean {
    ...
    protected onMessage(msg: any): void {
        ...
        let msgArr = Array.prototype.slice.call(new Int8Array(msg));
        let jsonObj = JSON.parse(utils.byteToString(msgArr.slice(8, msgArr.length)));
        ...
        if (this._requests.length > 0) {
            for (let reqIdx in this._requests) {
                ...
                if (req.rspCmd == rspCmd) {
                    ...
                    this._callbackExecuter(req.rspObject, jsonObj.data);
        ...
        if (utils.CheckNetMsgErrorCode(jsonObj.errcode)) {
            if (jsonObj.data) {
                console.log(jsonObj.data);
                this.dealSub(jsonObj.data);
            }
        }
    }
    ...
    public static subscribe(str: string, call: any, target: any) {
        let arr = str.split("."),
            fileName = arr[0],
            funcName = arr[1],
            wholeStr = fileName + "_" + funcName;
        if (null != fileName && null != funcName) {
            (NetNode._eventListeners[wholeStr] = {
                mod: fileName,
                type: funcName,
                handler: call,
                target: target
            ...
            cc.error("proto class is error!!!" + str);
        }
    }

    //分发回调
    private dealSub(json: any) {
        //优先分发item
        this.dealStrItem(json, 'item', 'time');
        //分发剩余的
        this.dealItem(json, 'item', 'time');
    }
    ...
    dealNumber(data: any) {
        ...
        for (let e in data) {
            ...
            if (null != o) {
                if ("number" != type) {
                    if ("object" == type) {
                        data[e] = this.dealNumber(o);
                    } else {
                        if ("string" == type && o.length < 11) {
                            data[e] = parseInt(o);
                        }
                    }
                }
    ...
    dealStrItem(data: any, ...arg) {
        ...
        for (let str of arg) {
            for (let i in data) {
                if (i == str) {
                    for (let tip in data[i]) {
                        let r = (data[i][tip]),
                            a = i + "_" + tip,
                            s = NetNode._eventListeners[a];
                        if (null != s) {
                            this.debugMode &&
                                cc.warn(
                                    "[NetNode]MOD:" + i + " type:" + tip + " accept"
                                );
                            if (this.debugMode) s.handler.apply(s.target, [r]);
                            else
                                try {
                            ...
                            this.debugMode &&
                                cc.warn(
                                    "[NetNode][NOT-EXISTS]MOD:" + i + " type:" + tip
                                );
                    }
                    break;
                }
    ...
    public send(buf: NetData, force: boolean = false, cmdId = 0, rspObject: CallbackObject = null): boolean {
        if (this._state == NetNodeState.Working || force) {
            console.log(`socket send ...`);
            if (rspObject && rspObject.callback) {
                this._requests.push({
                    buffer: buf,
                    rspCmd: cmdId,
                    rspObject: rspObject,
    ...
    public request(buf: NetData, rspCmd: number, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false) {
        ...
        // 进入发送缓存列表
        this._requests.push({
            buffer: buf, rspCmd, rspObject, isSend: false
        });
        // 启动网络请求层
        if (showTips) {
