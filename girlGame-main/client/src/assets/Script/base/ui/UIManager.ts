...
import { UILEVEL } from "../../game/ui/UIConfig";
...
export class UIManager {
    ...
    private updateUI() {
        ...
        for (let hide: number = hideIndex; hide < showIndex; ++hide) {
            if (this.UIStack[hide].uiView.uiLevel !== UILEVEL.TopUI)
    ...
    private onUIOpen(uiId: number, uiView: UIView, uiInfo: UIInfo, uiArgs: any) {
        ...
        switch (uiView.uiLevel) {
            case UILEVEL.BottomUI:
                child = child.getChildByName('BottomUI');
                break;
            case UILEVEL.CommonUI:
                child = child.getChildByName('CommonUI');
                break;
            case UILEVEL.TopUI:
                child = child.getChildByName('TopUI');
                break;
        }
        ...
        if (this.UIStack.length > 1) {
            ...
            for (let count = this.UIStack.length - 1; count >= 0; count--) {
                if (this.UIStack[count].uiView.uiLevel === UILEVEL.BottomUI && this.UIStack[count].uiView != uiView) {
                    fromUIID = this.UIStack[count].uiId;
                }
            }
        }
    ...
    public open(uiId: number, uiArgs: any = null, progressCallback: ProcessCallback = null): void {
        ...
        this.getOrCreateUI(uiId, progressCallback, (uiView: UIView): void => {
            ...
            // 打开UI，执行配置
            if (uiView.uiLevel === UILEVEL.BottomUI) {
                //BottomUI只允许存在一个
                this.closeAllBottom();
            }
    ...
    /** 关闭所有BottomUI界面 */
    public closeAllBottom() {
        try {
            let bottomUI = cc.director.getScene().getChildByName('Canvas').getChildByName('UI').getChildByName('BottomUI');
            for (let child of bottomUI.children) {
                this.close(child.getComponent('UIView'));
            }
        } catch (error) {
            console.error(error);
        }
