...
cc.Class({
  ...
  onLoad: function onLoad() {
    this.cellAreaArray(); //this.coordinateSystem()
    //this.test()
  ...
  test: function test() {
    var label = this.coordinateSystemNode.addComponent(cc.Label);
  ...
  coordinateSystem: function coordinateSystem() {
    //坐标系未完成
    // this.labelW = []
    var labelWArr = [];
    var labelWNodeArr = [];
    ...
    for (var i = 0; i < 80; i = i + 5) {
      var labelNode = this.coordinateSystemNode.addComponent(cc.Node);
      var label = labelNode.addComponent(cc.Label); // let label = labelW[i]
      // label.enable = true
      // label.enabled = true
    ...
    console.log(labelW); // for (let j = 0; j < this.maxHCnt; j++) {
    // }
  },
